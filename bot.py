...
# Конфигурация
BOT_TOKEN = "8222846517:AAEVXzNlrt14BWIK2QrA38JE72doZ2Fy3YE"
API_URL = "https://your-api.com/endpoint"  # Замените на URL вашего API


# Клавиатура с кнопкой авторизации
def get_main_keyboard():
    keyboard = [
        [KeyboardButton("🔐 Авторизоваться", web_app=WebAppInfo(url="https://your-webapp.com"))]
    ]
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "Все ваши действия будут отправлены на сервер."
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await message.reply_text(
        "✅ Ваше сообщение получено и отправлено на сервер!",
        reply_markup=get_main_keyboard()
    )
...
async def handle_web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Парсим initData
    try:
        # initData приходит в формате query string
        ...
        # Отправляем на API
        await send_to_api(webapp_data)

        await update.message.reply_text(
            "✅ Авторизация успешна! Данные отправлены на сервер.",
            reply_markup=get_main_keyboard()
        )
...
async def send_to_api(data: dict):
    ...
    #     )
    #
    #     print(f"📤 Отправлено на API: {data['action']}")
    #     print(f"📡 Status Code: {response.status_code}")
    #     print(f"📋 Response: {response.text}")
    #     print("=" * 50)
    #
    # except requests.exceptions.RequestException as e:
    #     logger.error(f"Ошибка отправки на API: {e}")
    # except Exception as e:
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update:
        print(f"Ошибка в update: {update}")
